...
export class AuthService {
  ...
  async register(options: IRegisterOptions): Promise<{ user: Partial<User>; token: string }> {
    ...
    console.log(`[注册] 开始初始化用户 ${user.id} 的配置`);
    await this.userService.initUserConfig(user.id);
    console.log(`[注册] 用户 ${user.id} 配置初始化完成`);
  ...
  async login(options: ILoginOptions): Promise<{ user: Partial<User>; token: string }> {
    ...
    if (wechatCode) {
      ...
      if (!user) {
        ...
        console.log(`[验证码登录] 开始初始化用户 ${user.id} 的配置`);
        await this.userService.initUserConfig(user.id);
        console.log(`[验证码登录] 用户 ${user.id} 配置初始化完成`);
      }
    ...
    if (!user || !user.id || !user.username) {
      console.error('[login] 用户对象异常:', JSON.stringify({
        hasUser: !!user,
        id: user?.id,
        username: user?.username,
        phone: user?.phone,
  ...
  private async handleWechatLogin(wechatCode: string, wechatInfo?: { unionid?: string; nickname?: string; avatarUrl?: string }): Promise<User | null> {
    ...
    if (!user) {
      ...
      console.log(`[微信登录] 开始初始化用户 ${user.id} 的配置`);
      await this.userService.initUserConfig(user.id);
      console.log(`[微信登录] 用户 ${user.id} 配置初始化完成`);
    } else if (wechatInfo) {
  ...
  async resetPassword(phone: string, code: string, newPassword: string): Promise<{ user: Partial<User>; token: string }> {
    const user = await this.userModel.findOne({ where: { phone } });
    if (!user) {
      throw new Error('该手机号未注册');
    }

    const codeValid = await this.smsService.verifyCode(phone, code, 'reset-password');
    if (!codeValid) {
      throw new Error('验证码无效或已过期');
    }
