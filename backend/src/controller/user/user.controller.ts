...
export class UserController {
  ...
  @Get('/config')
  async getConfig(): Promise<IApiResponse> {
    try {
      const userId = this.ctx.state.user?.userId;
      if (!userId) {
        return { success: false, message: '请先登录' };
      ...
      const configs = await this.userService.getUserConfig(userId);
  ...
  async updateConfig(@Body() body: {
    ...
    try {
      ...
      await this.userService.updateUserConfig(userId, body.configs);
