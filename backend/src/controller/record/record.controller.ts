...
export class RecordController {
  ...
  async getRecordsByAccount(
    ...
    try {
      ...
      const result = await this.recordService.getRecordsByAccountId(
        userId,
        parseInt(accountId),
        parseInt(page) || 1,
  ...
  async getLatestRecord() {
    try {
      ...
      const record = await this.recordService.getLatestRecord(userId);
      return { success: true, message: '获取成功', data: record };
    } catch (error) {
      return { success: false, message: '获取失败', error: error.message };
    }
  }
  ...
  async getNearestMonth(
    ...
    try {
      ...
      const result = await this.recordService.getNearestMonth(
        userId,
        yearMonth,
        direction
      );
