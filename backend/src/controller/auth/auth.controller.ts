...
export class AuthController {
  ...
  async sendSms(@Body() body: {
    ...
    try {
      await this.smsService.sendCode(body.phone, body.type);
      console.log(`\n========== 短信验证码 ==========`);
  ...
  async login(@Body() body: any): Promise<IApiResponse> {
    try {
      console.log('[login] 接收到body:', JSON.stringify(body));
  ...
  async resetPassword(@Body() body: {
    ...
    try {
      const result = await this.authService.resetPassword(body.phone, body.code, body.password);
      return {
        success: true,
        data: result,
        message: '密码重置成功',
